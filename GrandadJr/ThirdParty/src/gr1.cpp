...
struct _gr1 final : public ::faust::dsp {
	...
	void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Grain length (s)", &fHslider1, FAUSTFLOAT(0.1f), FAUSTFLOAT(0.001f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1e-06f));
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = fConst0 * std::max<float>(0.001f, std::min<float>(1.0f, float(fHslider1)));
	...
	FAUST_ADDHORIZONTALSLIDER("Grain length (s)", fHslider1, 0.1f, 0.001f, 1.0f, 1e-06f);
	...
	#define FAUST_LIST_ACTIVES(p) \
		...
		p(HORIZONTALSLIDER, Grain_length_(s), "Grain length (s)", fHslider1, 0.1f, 0.001f, 1.0f, 1e-06f) \
