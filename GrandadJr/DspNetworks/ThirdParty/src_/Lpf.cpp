...
struct _Lpf final : public ::faust::dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("name", "sallenKey2ndOrderLPF");
	...
	void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("sallenKey2ndOrderLPF");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp7 = float(input1[i0]) - (fRec5[1] + fTemp1 * fRec6[1]);
			float fTemp8 = fTemp0 * fTemp7 / fTemp3;
			float fTemp9 = fRec6[1] + 2.0f * fTemp8;
			float fRec4 = fRec5[1] + fTemp0 * fTemp9;
			float fTemp10 = fRec6[1] + fTemp8;
			fRec5[0] = fRec5[1] + 2.0f * fTemp0 * fTemp10;
			fRec6[0] = fTemp9;
